...
angle_arary =  np.linspace(40,61,60)



improv_matrix = np.zeros((len(angle_arary),len(astig_array)))
improv_matrixx= np.zeros((len(angle_arary),len(astig_array)))
improv_matrixy= np.zeros((len(angle_arary),len(astig_array)))
clrb_matrix_smlmz = np.zeros((len(angle_arary),len(astig_array)))
clrb_matrix_smlmy = np.zeros((len(angle_arary),len(astig_array)))
clrb_matrix_smlmx = np.zeros((len(angle_arary),len(astig_array)))
clrb_matrix_sfz = np.zeros((len(angle_arary),len(astig_array)))
clrb_matrix_sfx = np.zeros((len(angle_arary),len(astig_array)))
clrb_matrix_sfy = np.zeros((len(angle_arary),len(astig_array)))

check_esti = False

for angle_i in tqdm.tqdm(range(len(angle_arary))):
    for astig_i in tqdm.tqdm(range(len(astig_array))):

        with torch.no_grad():
            angle = angle_arary[angle_i]
...
for ii in range(np.size(improv_matrix,1)):
    x=np.flip(640 / np.absolute(1.33 * (1 - np.cos(np.arcsin(1.52 / 1.33 * np.sin(angle_arary / 180 * np.pi))))))
    y= np.flip(improv_matrix[:,ii]/1000)
    B_spline_coeff = UnivariateSpline(x, y,s=30)
    X_Final = np.linspace(x.min(), x.max(), 500)
    Y_Final = B_spline_coeff(X_Final)

    ax.plot(X_Final,
             Y_Final,label=r'$Z_2^2$ = '+str(int(astig_array[ii]/715*1000)) + r'm$\lambda$' )
...
plt.savefig('C:/Users/GrunwaldLab/Desktop/figures_paper_march12/astigmat_improv'+str(depth)+'.png', dpi=600)
plt.show()
