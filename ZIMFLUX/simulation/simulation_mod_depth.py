...
modulation = np.array([0.85, 0.95, 1]) # this is atigmatism actually
angle_beam =  np.geomspace(40,61,60) # actaully angle
depth = 300

improv_matrix = np.zeros((len(angle_beam),len(modulation)))
improv_matrixx= np.zeros((len(angle_beam),len(modulation)))
improv_matrixy= np.zeros((len(angle_beam),len(modulation)))
clrb_matrix_smlmz = np.zeros((len(angle_beam),len(modulation)))
clrb_matrix_smlmy = np.zeros((len(angle_beam),len(modulation)))
clrb_matrix_smlmx = np.zeros((len(angle_beam),len(modulation)))
clrb_matrix_sfz = np.zeros((len(angle_beam),len(modulation)))
clrb_matrix_sfx = np.zeros((len(angle_beam),len(modulation)))
clrb_matrix_sfy = np.zeros((len(angle_beam),len(modulation)))



for z_i in tqdm(range(len(angle_beam))):
    for modi in range(len(modulation)):

        with torch.no_grad():
            angle = angle_beam[z_i]
            ...
            mod=[]
            params = sim_params(depth=depth,astig=-60/1000*715 )
            dev = 'cuda'
            zstack = False
            ...
            Nbackground = torch.ones((numbeads,1))*7.6
            ...
            theta[:, 2] = angle_beam[z_i]
            theta[:, 3] = Nphotons[0]
            theta[:, 4] = Nbackground[0]


            num_patternstot = 3
            mod_contrast = modulation[modi]
...
ax.set_prop_cycle(color=['#d62728', '#9467bd', '#8c564b'])
...
for ii in range(np.size(improv_matrix,1)):
    ...
    ax.plot(X_Final,
            Y_Final*1000,label=r'$m$ = ' + str(modulation[ii]) )
...
plt.savefig(savepath +'modulation'+str(depth)+'.png', dpi=600)
plt.show()
