...
public:
	...
	cuCSplinePSF(Int2 roisize, const CSpline_Calibration& c, param_limits_t limits, int numconst) :
...
public:
	...
	CSplinePSF(Int2 roisize, const CSpline_Calibration& c, param_limits_t limits, int numconst) :
	...
	const float* coefs, int mode, bool cuda, Context* ctx)
{
	try {
		param_limits_t limits = {
			std::vector<float>{CSpline_Border, CSpline_Border, calib.z_min, 10.0f, 1.0f},
			std::vector<float>{roisize - 1 - CSpline_Border, roisize - 1 - CSpline_Border, calib.z_max, 1e9f, 1e9f}
		};

		if (mode == CSPLINE_TILTEDBG) {
			limits[0].push_back(-1e6);
			limits[0].push_back(-1e6);
			limits[1].push_back(1e6);
			limits[1].push_back(1e6);
		}
