...
public:
	...
	cuPSFImpl(SampleIndex roisize, param_limits_t limits, int numconst) :
		...
		int numspots, float * d_trace, int traceBufLen, cudaStream_t stream)  override
	{
		...
		LevMarSettings<Params::K> settings(limits[0], limits[1], lm_step_coeff, lm_params.iterations, lm_params.normalizeWeights);

		LaunchKernel(numspots, [=]__device__(int i) {
			TModel model(roisize, calib, &d_const[i*numconst]);
			Params initial_ = initial ? initial[i] : model.ComputeInitialEstimate(&d_sample[i * smpcount]);
			auto r = LevMarOptimize(&d_sample[i * smpcount], initial_, model, settings,
...
public:
	...
	PSFImpl(SampleIndex roisize, param_limits_t limits, int numconst) :
		...
		float* diag, int* iterations, int numspots, float * trace, int traceBufLen) override
	{
		...
		ParallelFor(numspots, [&](int i) {
			...
			auto r = LevMarOptimize(&samples[i * smpcount], ip, model,settings,
				&trace_[traceBufLen * i], traceBufLen);
...
public:
	...
	SimpleCalibrationPSF(const Calibration& calib, SampleIndex roisize, param_limits_t limits, int numconst) :
...
public:
	...
	virtual void SetLimits(const param_limits_t& lim) { psf->SetLimits(lim); }
	virtual param_limits_t GetLimits() { return psf->GetLimits(); }

	virtual void SetLMParams(LMParams p, std::vector<float> stepcoeff) { psf->SetLMParams(p, stepcoeff); }
	virtual std::pair<LMParams, std::vector<float> > GetLMParams() { return psf->GetLMParams(); }
