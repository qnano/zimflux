...
class GainOffset_Calib:
    ...
    def process_frame(self, img):
        return np.maximum( (np.array(img,dtype=np.float32)-self.offset)*self.gain, 0 )
...
class GainOffsetImage_Calib:
    def __init__(self, gain, offset, ctx):
        ...
        self.gain = np.ascontiguousarray(gain,dtype=np.float32)
        self.offset = np.ascontiguousarray(offset,dtype=np.float32)
        assert(np.array_equal(self.gain.shape,self.offset.shape))
        self.inst = self._GainOffsetImageCalib_Create(self.gain.shape[1], self.gain.shape[0], 
                                                      self.gain,self.offset,ctx.inst)
