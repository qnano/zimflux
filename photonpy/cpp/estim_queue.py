...
class EstimQueue_Results:
    ...
    def Filter(self, indices):
        ...
        if len(indices) != len(self.ids):
            print(f"Removing {len(self.ids)-len(indices)}/{len(self.ids)} based on xy position + iterations ")
    ...
    def Schedule(self, samples, roipos=None, ids=None, initial=None, constants=None):
        ...
        if constants is not None:
            ...
            if constants.size != numspots*self.estim.numconst:
                ...
                try:
                    constants_ = np.zeros([numspots,self.estim.numconst],dtype=np.float32)
                    constants_[:,:] = constants
                    constants = constants_
                except:                
                    raise ValueError(f'Provided constants with shape {constants.shape}, expecting {[numspots,self.estim.numconst]}')
        else:
