...
from photonpy.cpp.lib import SMLMLib
import tqdm
...
class SpotList:
    ...
    def get_xyI(self, ep, normalize=False, pieterload=False):
    ...
    def render_sr_image_per_pattern(self, ep, frames, sr_factor, ctx: Context, debug_images=False):
        ...
        xyI, z = self.get_xyI(ep, normalize=ep >= 0, pieterload=True)
        xyI = xyI[indices]
        z = z[indices]  # bs

        h, w = self.ds.imgshape
        img = np.zeros((h * sr_factor, w * sr_factor), dtype=np.float32)
        img = self._draw_spots(img, xyI[:, 0], xyI[:, 1], xyI[:, 2], sr_factor, ctx)
    ...
    def estimate_angle_and_pitch(self, pattern_frames, frame_bins, ctx, freq_minmax, debug_images,
                                 dft_peak_search_range):
    ...
    def estimate_phase_and_depth_old(self, mod_list, frame_bins, method=0):
        ...
        for b, fr_bin in enumerate(frame_bins):
            for m, mod in enumerate(mod_list):
                ...
                if method == 0:
                    phase[b, m], depth[b, m], relint[b, m] = self._estimate_phase_and_depth(mod, m, pattern_frames,
                                                                                            fr_bin)
                else:
                    phase[b, m], depth[b, m], relint[b, m] = self._estimate_phase_and_depth2_old(mod, m, pattern_frames,
                                                                                                 fr_bin)
    ...
    def estimate_phase_and_depth(self, k_list, pattern_frames, frame_bins, percentile=50, iterations=3):
        ...
        for b, fr_bin in enumerate(frame_bins):
            for ax, patidx in enumerate(pattern_frames):
                for i in patidx:
                    r = self._estimate_phase_and_depth2(k_list[i], i, fr_bin, percentile, iterations)
    ...
    def plot_intensity_variations(self, mod, minfilter, pattern_frames):
        ...
        mod_ = mod[np.newaxis]
        exc = mod_[:, :, 4] * (1 + mod_[:, :, 2] * np.sin(
            xyI[:, :, 0] * mod_[:, :, 0] + xyI[:, :, 1] * mod_[:, :, 1] - mod_[:, :, 3]))
        ...
        for axis in range(len(pattern_frames)):
            ...
            plt.figure()
            plt.hist([expectedRelErr[:, fr].flatten(), measuredRelErr[:, fr].flatten()],
                     label=['Expected rel err: CRLB(N_k) / (P_k*N)', 'Measured err: |N_k - P_k*N|/(P_k*N)'],
                     range=(0, 2), bins=50)
    ...
    def _estimate_phase_and_depth2_old(self, mod, ep, pattern_frames, frames):
    ...
    def _estimate_phase_and_depth2(self, k, ep, frames, percentile=50, iterations=10):
        ipf = self.ds.indicesPerFrame()
        indices = np.concatenate([ipf[i] for i in frames])

        #        sumI = np.sum(self.IBg[indices][: , pattern_frames, 0],1)
        sumI = np.sum(self.IBg[indices][:, :, 0], 1)
        # sumI = self.ds.photons[indices]
        ...
        for it in range(iterations):
            ...
            q = relint * (1 + depth * np.sin((k[None] * pos[:, :len(k)]).sum(1) - phase))

            normI = modulatedIntensity / sumI
            errs = np.abs(normI - q)

            median_err = np.percentile(errs, percentile)  # median(errs)
            weights = errs < median_err

            trace.append(f"{depth:.2f}")

        # print(f"Phase estimation depth per iteration: {','.join(trace)}")
    ...
    def refine_pitch(self, mod, ctx: Context, spotfilter, plot=False):
        ...
        for k in range(2):
            ...
            # Adjust pitch
            print(
                f"Adjust kx: {cx[0] * kx:.4f}, ky: {cy[0] * ky:.4f}. phase shift x: {cx[1] * kx:.4f}, phase shift y: {cy[1] * ky:.4f}")
    ...
    def bias_plot2D(self, mod, ctx: Context, spotfilter, tag):
        ...
        plt.hist([xysx[:, 2] * self.pixelsize, xysy[:2] * self.pixelsize], label=['X bias', 'Y bias'], range=[-50, 50],
                 bins=50)
    ...
    def draw_spots_in_pattern(self, filename, allmod, ep, tiffname, spotfilter, numpts=2000, closefig=True):
    ...
    def excitation_matrix(self, mod, sel):
        ...
        ky = mod_['k'][:, :, 1]
        exc = mod_[:, :]['relint'] * (
                    1 + mod_[:, :]['depth'] * np.sin(xyI[:, :, 0] * kx + xyI[:, :, 1] * ky - mod_['phase']))
        return exc
