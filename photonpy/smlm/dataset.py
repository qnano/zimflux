...
class Dataset:
    ...
    def estimateDriftRCC(self, framesPerBin=500, zoom=1, maxdrift=3):
        ...
        if self.dims == 2:
            return rcc(self.pos, self.frame, int(self.numFrames/framesPerBin), 
                        maxdrift=maxdrift, zoom=zoom)[0]
        
        else:
            assert self.dims==3
            return rcc3D(self.pos, self.frame, int(self.numFrames/framesPerBin), zoom=zoom)
    ...
    def cluster(self, maxDistance, debugMode=False):
        ...
        print("Computing cluster properties")
        dt = self.data.estim.dtype
        counts = np.bincount(mapping)
        def getClusterData(org):
            r = np.recarray( len(counts), dtype=dt)
    ...
    def saveHDF5(self, fn, saveGroups=False, fields=None):
        ...
        with h5py.File(fn, 'w') as f:
            ...
            locs['x'] = self.pos[:,0]*65
            locs['y'] = self.pos[:,1]*65
            ...
            if self.dims==3:
                locs['z'] = self.pos[:,2] *1000
                locs['lpz'] = self.crlb.pos[:,2] *1000
    ...
    def loadHDF5(fn, **kwargs):
        ...
        with h5py.File(fn, 'r') as f:
            ...
            ds = Dataset(len(locs), dims, imgshape, haveSigma = haveSigma, **kwargs)
    ...
    @classmethod
    def fromEstimates(cls, estim, param_names, framenum, imgshape, crlb=None, chisq=None,iterations=None,
    ...
    def fromQueueResults(qr, imgshape, **kwargs) -> 'Dataset':
        return Dataset.fromEstimates(qr.estim,  qr.param_names, qr.ids, imgshape, qr.crlb, qr.chisq, roipos=qr.roipos, iterations=qr.iterations, **kwargs)
