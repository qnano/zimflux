...
def crosscorrelation_cuda(A, B, smlm: SMLMLib):
    return smlm.IFFT2(np.conj(smlm.FFT2(A)) * smlm.FFT2(B))



def findshift(cc, smlm:SMLMLib, plot=False):
...
def findshift_pairs(images, pairs, smlm:SMLMLib, useCuda=True):
...
def rcc(xy, framenum, timebins, maxdrift=3, zoom=1, 
        sigma=1, maxpairs=1000,RCC=True,smlmlib:SMLMLib=None):
 ...
 #   area = np.array([area[0],area[0]])
    ...
    with Context(smlmlib) as ctx:
        ...
        for k in range(timebins):
            ...
            spots[:, 0] = (xy[indices,0] * zoom)# % imgshape[1]
            spots[:, 1] = (xy[indices,1] * zoom) #% imgshape[0]
            spots[:, 2] = sigma
            spots[:, 3] = sigma
            spots[:, 4] = 1
        ...
        if RCC:
            ...
            pair_shifts = findshift_pairs(images, pairs, ctx.smlm)
...
def rcc3D(xyzI, framenum, timebins, zoom, smlmlib:SMLMLib=None,sigma=1):
    
    drift_xy = rcc(xyzI[:,[0,1,3]], framenum, timebins, smlmlib=smlmlib, sigma=sigma)[0]

    sheared = xyzI[:,[0,1,3]]*1
    sheared[:,1] += xyzI[:,2]
    drift_sheared = rcc(sheared, framenum, timebins, smlmlib=smlmlib, sigma=sigma)[0]

    drift_xyz = np.zeros((len(drift_xy),3))
    drift_xyz[:,:2] = drift_xy
    drift_xyz[:,2] = drift_sheared[:,1] - drift_xy[:,1]
    return drift_xyz
...
def minEntropy(positions, framenum, crlb, framesperbin, imgshape, 
          ...
          maxspots=None, initializeWithRCC=True, initialEstimate=None, rcc_zoom=2,estimatePrecision=True,
          maxNeighbors=5000):
    """
    ...
    if initialEstimate is not None:
        ...
        if type(initializeWithRCC) == bool:
            initializeWithRCC = numframes//4

        posI = np.ones((len(positions),positions.shape[1]+1))
        posI[:,:-1] = positions[:,:]

        timebins = numframes // initializeWithRCC
        if positions.shape[1] == 3:
            initial_drift = rcc3D(posI, framenum, timebins, zoom=rcc_zoom)
        else:
            initial_drift = rcc(posI, framenum, timebins, zoom=rcc_zoom)[0]
    ...
    with Context(debugMode=debugMode) as ctx:
        ...
        if estimatePrecision:
            ...
            est_precision = np.std(diff,0)/np.sqrt(2)
        ...
        if display:
            ...
            def plotwnd(L):
                ...
                if outputfn is not None:
                    plt.savefig(os.path.splitext(outputfn)[0] + ".png", dpi=600)
                    plt.savefig(os.path.splitext(outputfn)[0] + ".svg", dpi=600)
