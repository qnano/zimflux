    ...
    for i in tqdm.trange(len(photons)):
        ...
        if True:            
            if i == len(photons)-1:
                ...
                plot_traces(traces[:20], thetas_[:20], psf, axis=I_ix)
...
with Context(debugMode=False) as ctx:
    ...
    roisize=16
    bg=10
    numspots = 50 if ctx.smlm.debugMode else 500
    ...
    print(g_psf.limits)
    g_psf.SetLevMarParams()
    
    g_psf.SetLevMarParams(1e2, normalizeWeights=True, iterations=200)
    g_s_psf = g.CreatePSF_XYIBgSigma(roisize, sigma+1, useCuda)
    g_s_psf.SetLevMarParams(1e3,200)
    g_sxy_psf = g.CreatePSF_XYIBgSigmaXY(roisize, [sigma+1,sigma+1], useCuda)
    g_sxy_psf.SetLevMarParams(1e3,200)
    g_z_psf = g.CreatePSF_XYZIBg(roisize, calib, useCuda)
    g_z_psf.SetLevMarParams(1e2, 200)
    ...
    data = [
        ...
        (g_s_psf, '2D Gauss Sigma', estimate_precision(g_s_psf, g_s_psf.Estimate, theta_sig[:,:5], photons)),
        (g_sxy_psf, '2D Gauss Sigma XY', estimate_precision(g_sxy_psf,g_sxy_psf.Estimate, theta_sig, photons)),
         ]
    ...
    if cspline_fn is not None:
        ...
        cs_psf = CSplineMethods(ctx).CreatePSF_XYZIBg(roisize, cs_calib, fitMode=CSplineMethods.FlatBg, cuda=True)
        cs_psf.SetLevMarParams(1e4, 300)
